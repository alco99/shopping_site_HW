				...
				echo '<div class="row">';//購物車產品列
					...
					if($cart){
						...
						for($i = 0; $i < count($cart["ID"]); $i++){
							...
							echo '<div class="col-2">';
								$stock = getProductformID($cart['ProductID'][$i]);
								if($cart["ProductQuantity"][$i] > $stock['InStock'][0]){//購買數量大於庫存
									UpdateCartQ($_SESSION['UserID'], $cart['ProductID'][$i], $stock['InStock'][0]);
									echo '庫存不足已修正：'.$stock['InStock'][0];
								} else {
